...
use Elevate\Collections\Services\FilterService;
...
class CollectionWebController extends Controller
{
    ...
    public function show(Request $request, string $parent = null, ?string $child = null, ?string $filters = null)
    {
        ...
        if ($collection->template) {
            ...
            $defaultTemplate = \Elevate\Editor\Models\Template::where('model_type', 'Elevate\Product\Models\Collection')
        ...
        // Build query for collection items
        // For now, we'll assume products - you can extend this for other types
        $itemsQuery = $collection->products()->getQuery();
        
        // Apply filters to query
        if (!empty($appliedFilters)) {
            $itemsQuery = $this->filterService->applyFilters($itemsQuery, $appliedFilters);
        }
        ...
        $availableFilters = $this->filterService->getAvailableFilters(
            $collection,
            $appliedFilters,
            $collection->products()->getQuery()
        );

        // Add data to collection for template access
        $collection->items = $items;
        $collection->products = $items; // Also set as products for backward compatibility
