<x-app pageTitle="Edit Product" title="Edit Product - Admin">
...
<div class="space-y-6">
    ...
    <div class="flex justify-between items-center">
        ...
        <a href="{{ route('products.index') }}" class="inline-flex items-center px-4 py-2 bg-white border border-gray-300 rounded-lg text-sm font-medium text-gray-700 hover:bg-gray-50 transition-colors">
            ...
            <div class="lg:col-span-2 space-y-6">
                    ...
                    <div class="space-y-4">
                        ...
                        <div>
                            ...
                            <textarea name="short_description" id="short_description" rows="2"
                                class="block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 sm:text-sm @error('short_description') border-red-300 @enderror">{{ old('short_description', $product->short_description) }}</textarea>
                        ...
                        <div>
                            ...
                            <textarea name="description" id="description" rows="6"
                                class="block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 sm:text-sm @error('description') border-red-300 @enderror">{{ old('description', $product->description) }}</textarea>
                            @error('description')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        </div>
                ...
                <div class="bg-white rounded-lg border border-gray-200 p-6">
                    ...
                    <div class="space-y-4">
                        ...
                        {{-- Stock --}}
                        <div id="stock-field" style="display: {{ old('track_inventory', $product->track_inventory) ? 'block' : 'none' }};">
                ...
                <div class="bg-white rounded-lg border border-gray-200 p-6">
                    ...
                    <div class="space-y-4">
                        {{-- Requires Shipping --}}
                        <div class="flex items-center">
                            <input type="checkbox" name="requires_shipping" id="requires_shipping" value="1" {{ old('requires_shipping', $product->requires_shipping) ? 'checked' : '' }}
                                class="h-4 w-4 text-blue-600 focus:ring-blue-500 border-gray-300 rounded">
            ...
            <div class="space-y-6">
                ...
                <div class="bg-white rounded-lg border border-gray-200 p-6">
                    ...
                    <div class="space-y-4">
                        ...
                        @if($product->type === 'variable')
                            <div class="pt-2">
                                <a href="{{ route('products.variants', $product) }}" class="inline-flex items-center px-4 py-2 bg-purple-600 text-white text-sm font-medium rounded-lg hover:bg-purple-700 transition-colors w-full justify-center">
                                    <svg class="w-5 h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    <div>
                        ...
                        <select name="template_id" id="template_id"
                            ...
                            @foreach($templates as $template)
                                <option value="{{ $template->id }}" {{ old('template_id', $product->template_id) == $template->id ? 'selected' : '' }}>
                                    {{ $template->name }}
                                </option>
                            @endforeach
                        </select>
                ...
                <div class="bg-white rounded-lg border border-gray-200 p-6">
                    ...
                    <x-commerce::tag-selector 
                        :allTags="$allTags" 
                        :selectedTags="$product->tag_values ?? []" 
                        name="tags"
                    />
                </div>
                ...
                <div class="bg-white rounded-lg border border-gray-200 p-6">
                    ...
                    <div class="space-y-4">
                        <div class="flex items-center">
                            <input type="checkbox" name="is_taxable" id="is_taxable" value="1" {{ old('is_taxable', $product->is_taxable) ? 'checked' : '' }}
                                class="h-4 w-4 text-blue-600 focus:ring-blue-500 border-gray-300 rounded">
                        ...
                        <div id="tax-rate-field" style="display: {{ old('is_taxable', $product->is_taxable) ? 'block' : 'none' }};">
                            <label for="tax_rate" class="block text-sm font-medium text-gray-700 mb-1">
                                Tax Rate
                            </label>
                            <div class="relative">
                                <input type="number" name="tax_rate" id="tax_rate" value="{{ old('tax_rate', $product->tax_rate) }}" step="0.01" min="0" max="1"
